export function imgVer(Config) {
    ...
    var left_Num = -X + 10;
    var html = '<div style="position:relative;padding:15px;border:1px solid #ddd;background:#ffffff;">';
    html += '<p class="ver-close" style="position: absolute;right: 5px;top: 2px;cursor: pointer;"><img src="../../images/slideverification/slideverification-x.png"/></p>';
    html += '<div style="margin-top: 10px;"><p class="ver-tips">Complete the image</p>';
    html += '<div class="re-btn"><a></a><span>Refresh</span></div></div>';
    html += '<div style="position:relative;overflow:hidden;width:' + w + 'px;margin: 5px 0 15px 0;">';
    ...
    html += '<div style="border:1px solid #c3c3c3;border-radius:24px;background:#ffffff;box-shadow:0 1px 1px rgba(12,10,10,0.2) inset;">';
    html += '<p class="mwCompleteValidation" style="font-size:12px;color: #486c80;line-height:28px;margin:0;text-align:center;padding-right:22px;">Swipe to complete puzzle</p>';
    html += '</div>';
    ...
    onmousemove = function (e) {
        ...
        $(".ver-close").click(function () { // 关闭图片层
            $(".mwVerBox").hide();
        });
    };
    onmouseup = function (e) {
        ...
        if (moveStart == '') {
            ...
            if (Max_left > moveEnd_X && moveEnd_X > Min_left) {
                $(".mwCompleteValidation").html("完成拼图");
                ...
                if ($("#mwInputEmail").val() != '' && !emailReg.test($("#mwInputEmail").val())) {
                    $(".mwVerifyEmail span").text('Please enter a valid email address');
                    $(".mwVerifyEmail").show();
                } else if ($("#mwInputPassword").val() != '' && ($("#mwInputPassword").val().length < 6 || $("#mwInputPassword").val().length > 22)) {
                    $(".mwVerifyPwd span").text('Enter 6-20 characters');
                    $(".mwVerifyPwd").show();
                } else {
                    $(".mwVerifyPwd").hide();
